...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: lb [options] <port-mapping> [<port-mapping>...]\n\n")
		fmt.Fprintf(os.Stderr, "A lightweight TCP/HTTP/HTTPS load balancer with dynamic DNS resolution.\n\n")
		fmt.Fprintf(os.Stderr, "Port mapping format:\n")
		fmt.Fprintf(os.Stderr, "  [listen_port:]host:backend_port[,options]\n")
		fmt.Fprintf(os.Stderr, "  [listen_port[-port2]:]host:backend_port[-port2][,options]\n")
		fmt.Fprintf(os.Stderr, "  [listen_port:]host1:port1+host2:port2[,options]\n\n")
		fmt.Fprintf(os.Stderr, "Per-mapping options (comma-separated):\n")
		fmt.Fprintf(os.Stderr, "  http                   Enable HTTP mode with cookie-based affinity\n")
		fmt.Fprintf(os.Stderr, "  https                  Enable HTTPS mode with TLS\n")
		fmt.Fprintf(os.Stderr, "  lb=<algorithm>         Load balancing: random, round-robin, least-connection, weighted-random\n")
		fmt.Fprintf(os.Stderr, "  affinity               Enable IP-based session affinity\n")
		fmt.Fprintf(os.Stderr, "  proxy-server=<v1|v2>   Expect PROXY protocol from upstream\n")
		fmt.Fprintf(os.Stderr, "  proxy-client=<v1|v2>   Send PROXY protocol to backend\n\n")
		fmt.Fprintf(os.Stderr, "Options:\n")
		flag.CommandLine.VisitAll(func(f *flag.Flag) {
			fmt.Fprintf(os.Stderr, "  --%-24s %s", f.Name, f.Usage)
		...
		fmt.Fprintf(os.Stderr, "\nExamples:\n")
		fmt.Fprintf(os.Stderr, "  # TCP load balancing\n")
		fmt.Fprintf(os.Stderr, "  lb 8080:backend:9000\n\n")
		fmt.Fprintf(os.Stderr, "  # HTTP with cookie affinity\n")
		fmt.Fprintf(os.Stderr, "  lb 8080:backend:9000,http\n\n")
		fmt.Fprintf(os.Stderr, "  # HTTPS with auto-generated certificate\n")
		fmt.Fprintf(os.Stderr, "  lb 8443:backend:9000,https\n\n")
		fmt.Fprintf(os.Stderr, "  # Least-connection with IP affinity\n")
		fmt.Fprintf(os.Stderr, "  lb 8080:backend:9000,http,lb=least-connection,affinity\n\n")
		fmt.Fprintf(os.Stderr, "  # Port range mapping (1:1)\n")
		fmt.Fprintf(os.Stderr, "  lb 8080-8090:backend:9000-9010,http\n\n")
		fmt.Fprintf(os.Stderr, "  # Port range fan-in (many listen ports to one backend port)\n")
		fmt.Fprintf(os.Stderr, "  lb 30000-32000:backend:9090\n\n")
		fmt.Fprintf(os.Stderr, "  # Multi-target: one port, multiple backends\n")
		fmt.Fprintf(os.Stderr, "  lb 8080:service1:9000+service2:9001,http\n\n")
		fmt.Fprintf(os.Stderr, "  # Multi-target: same host, different ports\n")
		fmt.Fprintf(os.Stderr, "  lb 8080:backend:9000+9001+9002\n\n")
		fmt.Fprintf(os.Stderr, "  # PROXY protocol to backend\n")
		fmt.Fprintf(os.Stderr, "  lb 8080:backend:9000,proxy-client=v2\n\n")
		fmt.Fprintf(os.Stderr, "  # Multiple mappings with different configs\n")
		fmt.Fprintf(os.Stderr, "  lb --verbose --affinity-ttl 60s \\\n")
		fmt.Fprintf(os.Stderr, "    8080:web:9000,http,lb=least-connection \\\n")
		fmt.Fprintf(os.Stderr, "    8443:web:9000,https \\\n")
		fmt.Fprintf(os.Stderr, "    9090:api1:8080+api2:8080,http,affinity\n")
	}
